...
**成员A（合约）：** 负责 ZetaChain 上的智能合约开发（跨链支付、积分系统）。

**成员B（后端/Agent）：** 负责后端 API 和核心的 **Qwen-Agent** 集成（任务解析、流程控制）。

**成员C（前端/Pitch）：** 负责用户界面和最终的演示材料（Pitch 视频、演讲稿）。

**2\. 每日里程碑：**

**Day 1：** 定技术栈、写合约初版、设计API、搭前端框架。

**Day 2：** 部署测试合约、实现基础API与Agent、前端连接钱包。

**Day 3：** 实现完整跨链支付与Agent对话、前端任务流、录制Demo初稿。

**Day 4：** 端到端测试、优化、完善Pitch材料、最终提交。

二、Demo 脚本草稿

**1\. 开场价值主张：**

TaskPilot——用一句话发布跨链悬赏任务的去中心化平台。

**2\. 用户操作流程（2-3步）：**

**步骤一（发布）：** 用户输入“设计一个Logo，预算50 USDC”。

**步骤二（支付）：** 用户用以太坊的USDC支付。（这里是 ZetaChain 发力点：实现跨链锁仓，执行者可在Polygon接收USDT）。

**步骤三（完成）：** 执行者提交作品，审核后资金自动释放并颁发链上积分。
...
\- **多链DeFi用户**：需要监控各链授权合约风险

\- **项目方**：监控跨链合约异常调用

关键功能

1\. **交易预检**：AI分析合约风险、Gas估算、成功率预测

2\. **实时监控**：监控多链异常交易与授权变化

3\. **即时告警**：高风险操作实时通知+紧急处理建议
...
\- Function calling解析交易数据

\- 工具调用集成链上数据

\- 多语言报告生成

\- **支持3链**：ETH、Polygon、BSC

\- **监控类型**：跨链转账、合约调用、资产授权

\- **告警方式**：Telegram + 网页仪表板

\- **目标**：风险检测准确率>85%，响应时间<30秒
...
实现了自然语言→Agent→后端→链上查询的通路

创建了ZetaChain测试网余额查询的完整工作流
...
2\. AI代理层：QwenAgent解析并生成结构化JSON

3\. 后端服务层：Flask服务处理链上RPC调用

4\. 区块链层：ZetaChain测试网(Athens) EVM RPC
...
    "action": "query\_balance",

    "chain": "zetachain\_testnet",

    "address": "0x..."

  }

多链RPC配置：支持以太坊、BNB Chain和ZetaChain测试网

三、已实现功能验证

可执行的操作流

1\. 用户输入："查一下ZetaChain测试网上0x123...的ZETA余额"

2\. Agent输出：结构化JSON参数

3\. 后端响应：连接RPC并返回余额数据

4\. 控制台输出：\[后端执行日志\] 计划在链 zetachain\_testnet 上查询地址 0x123... 的余额
...
2\. 合约交互：集成ZetaChain跨链合约调用

3\. 前端界面：添加简单Web界面增强展示效果

五、学习心得

今日完成了从概念到可运行Demo的跨越，最重要的是打通了自然语言→结构化参数→链上操作的关键链路。虽然当前版本只实现了查询，但架构设计已为后续添加真实交易功能预留了接口，符合“最小可用版”的学习目标。
