...
-   DeFi 新手（痛点：费用高、步骤太复杂）
    
-   DeFi 高级玩家（痛点：想自动化优化收益）
...
> “用户只说需求，AI 自动找链、找池、找利率、并通过 ZetaChain 执行。”
...
-   工具链（parse\_intent / get\_yield / swap / execute）
    
-   决定最佳链 / 最佳收益池 / 最佳 swap 路径
...
@app.post("/swap")
def do_swap(intent: SwapIntent):
    print(f"[AI Agent] 准备执行 swap —— 链={intent.chain}, 用 {intent.amount} {intent.tokenIn} 换 {intent.tokenOut}")

    return {
        "status": "ok",
        "message": f"模拟交易: {intent.amount} {intent.tokenIn} → {intent.tokenOut} @ {intent.chain}"
    }
