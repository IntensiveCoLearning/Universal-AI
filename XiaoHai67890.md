...
**项目名称**：Give Me A Cola（全链创作者打赏平台）  
**一句话 Slogan**：

> 一杯可乐的钱，让全球粉丝用任意链为你发电。

**电梯陈述（30 秒版）**  
现有的创作者打赏平台（如 Buy Me a Coffee）以法币为主，Web3 创作者要收打赏，要么自己搭页面、要么在不同链上发很多地址，极度割裂，也无法覆盖 BTC 生态粉丝。

**Give Me A Cola** 利用 **ZetaChain 全链智能合约**，为创作者提供一个「**全链统一打赏链接**」，支持包括比特币在内的多链资产；同时接入 **Qwen / Qwen-Agent** 作为链下 AI 助手，帮助创作者做内容安全、粉丝互动和收入分析，让打赏体验更轻、更安全、更可持续。
...
1.  **Web3 原生创作者**
    ...
    -   Twitter/X KOL、加密 YouTuber、Space 主持人
        
    -   目前常用「钱包地址贴 Bio」或「手动发收款地址」，体验粗糙、数据分散
        
2.  **开源开发者 / Web3 Dev**
    
    -   想获得链上赞助的 GitHub 开发者，GitHub Sponsors 以法币和特定地区为主；
        
    -   希望粉丝可以用 ETH / BTC / 其他链资产直接打赏，作为 GitHub Sponsors 的加密替代。
        
3.  **泛内容创作者**
    
    -   播客、写作者、设计师、Newsletter 作者
        
    -   想增加一条加密打赏渠道，拓展全球用户，不依赖单一支付渠道。
        

### 2.2 典型使用场景

-   X 上的加密 KOL 在置顶 tweet 中放一个「🍹Give me a cola」链接：
    
    -   粉丝点开 → 选择自己熟悉的链或 BTC → 一键打赏。
        
-   GitHub README 中放一个「给我一杯可乐」按钮：
    
    -   赞助人可直接用多链资产支持这个仓库的维护。
        
-   Newsletter / 个人博客页加入打赏部件：
    
    -   读者在阅读时顺手打赏，不用跳转复杂的 Web3 界面。
...
-   创作者只需 **连接一次钱包 → 生成一个全链打赏链接**。
    
-   粉丝端可以从任意支持链（EVM 链 + BTC 等）发起打赏，后端由 **ZetaChain 全链智能合约统一收口**：
    
    -   使用 ZetaChain 的 **omnichain smart contract / ZRC-20 机制**，在单一链上就能管理跨链资产，包含 BTC 在内的原生代币。
        
-   创作者端可选择资金最终归集到哪条主链。
    

> 对标：Buy Me a Coffee 的「一个链接收全世界法币」；我们是「一个链接收全链加密资产」。

### 3.2 创作者主权 & 钱包原生身份

-   创作者以 **钱包地址** 作为主身份，而不是平台账号：
    
    -   打赏记录与粉丝关系可写入 ZetaChain / 其他链上，降低平台锁定。
        
-   创作者主页包含：打赏入口、作品链接、社交账号、链上打赏历史聚合。
    
-   可以导出打赏者地址列表（可选匿名聚合），用于空投、NFT Pass、社区分级等后续运营。
    

### 3.3 AI 加持：安全、助手、推荐（Qwen / Qwen-Agent）

接入 **Qwen / Qwen-Agent** 作为链下 AI 层，利用其工具调用和规划能力：

1.  **内容安全 & 风控助手**
    
    -   对创作者自填的简介、打赏文案做轻量内容审核与优化；
        
    -   识别疑似诈骗 / 钓鱼型创作者，给出风险提示；
        
2.  **创作者运营助手**
    
    -   根据打赏数据，为创作者生成「本周打赏报告」和「推荐感谢文案」；
        
    -   自动生成个性化的「打赏文案 A/B 测试版本」，提高转化率。
        
3.  **智能推荐（可做为进阶版）**
    
    -   聚合用户的链上打赏偏好和浏览轨迹，为粉丝推荐可能感兴趣的创作者。
        

## 4\. 技术路线（细化版）

### 4.1 区块链层：ZetaChain 全链合约

-   使用 **ZetaChain** 作为核心结算与逻辑链，编写 **omnichain smart contract**：
    
    -   支持从多条外部链（包括 Bitcoin 等非智能合约链）发起打赏请求；
        
    -   合约负责：
        
        -   记录打赏事件（打赏人地址、金额、来源链、目标创作者）
            
        -   将资金路由 / 归集到创作者设定的主链或子地址
            
-   可参考 ZetaChain 官方示例仓库中的跨链转账、ZRC-20 资产转移逻辑。
    

### 4.2 前端：Next.js 14 + TypeScript + TailwindCSS

-   使用 **Next.js 14 App Router** & React Server Components 构建创作者主页 & 打赏页面。
    
-   UI：TailwindCSS 打造轻量风格，方便在黑客松期间快速迭代。
    
-   钱包连接：
    
    -   使用 **RainbowKit + Wagmi** 提供优雅的钱包连接体验；RainbowKit 为钱包 UI，Wagmi 提供 EVM 交互 Hooks。
        

### 4.3 后端：Node.js / Express + PostgreSQL

-   **API 网关**：对接前端、Qwen-Agent、ZetaChain 节点 / RPC。
    
-   **存储**（PostgreSQL）：
    
    -   创作者配置（头像、描述、绑定社交链接）
        
    -   打赏记录索引（便于检索 & 统计），链上数据仍以合约为准
        
-   可扩展队列（如简单的任务表）用于处理异步操作：发送通知、生成 AI 报告等。
    

### 4.4 Web3 基础设施：RainbowKit + Wagmi + Alchemy AA

-   **RainbowKit + Wagmi**：前端钱包连接 & 调用合约。
    
-   **Alchemy AA（Account Abstraction）**：
    
    -   针对「不了解 gas / 没有原生资产」的粉丝，利用 AA + Paymaster 方案，让他们以更友好的方式完成打赏，如：代付 gas / 统一到账户体验。
        

### 4.5 千问：Qwen / Qwen-Agent 作为链下 AI 层

-   基于 **Qwen 大模型** + **Qwen-Agent 框架**：
    
    -   通过工具调用能力，将「打赏数据查询」「内容生成」「审核服务」封装为工具函数。
        
-   Worker 模式：
    
    -   定时任务拉取链上/数据库数据 → 调用 Qwen-Agent → 生成周报 / 文案 → 存回数据库供前端展示。
...
**对标产品：Buy Me a Coffee（Web2）**

-   优点：一键打赏、多种收益模式（一次性打赏、会员、商店）、体验成熟。
    
-   不足：
    
    -   偏 Web2 / 法币支付，对加密资产支持有限；
        
    -   无法原生接入链上身份、NFT、链上行为等。
        

**Give Me A Cola 的差异点：**

1.  **全链原生**：支持 BTC + 多链资产，创作者无需同时维护多条链地址。
    
2.  **链上身份 & 受众关系可携带**：钱包即身份，打赏记录可用于后续空投 / NFT 权益。
    
3.  **AI 智能助理**：帮助创作者优化运营，而不是单纯收款链接。
    

## 6\. Demo 流程（黑客松版本）

> 控制在 2–3 分钟内，突出 ZetaChain & Qwen 发力点。

**Step 1：创作者创建全链打赏链接（ZetaChain 发力点）**

1.  创作者用 RainbowKit 连接钱包 → 授权创建「创作者主页」。
    
2.  选择想要接受打赏的「目标主链 + 代币」，例如：
    
    -   主链：Ethereum / Polygon / ZetaChain
        
    -   接受代币：USDC / ETH / BTC 对应的 ZRC-20 映射资产等。
        
3.  点击「生成我的 Cola Link」，合约在 ZetaChain 上为该钱包生成一个全链收款入口。
    

> 这里明确说：
> 
> -   **ZetaChain 发力点 #1**：全链合约一次部署，就能处理来自多条链、包括 BTC 在内的打赏请求。
>     

**Step 2：粉丝从任意链打赏（ZetaChain 再次发力）**

1.  Demo 中展示粉丝点开链接：
    
    -   在前端选择「我想用：BTC / ETH / BNB / Polygon」之一；
        
2.  粉丝发起一笔打赏交易：
    
    -   对于 EVM 链：通过 Wagmi 直接发交易；
        
    -   对于 BTC：通过 ZetaChain 机制，将 BTC 转为合约可识别的 ZRC-20 资产并路由。
        
3.  成功后，在创作者端页面实时显示这笔打赏记录（金额、来源链、留言）。
    

> **ZetaChain 发力点 #2**：
> 
> -   打赏资金由 ZetaChain 全链合约统一接收与结算，支持 BTC 等非智能合约链资产。
>     

**Step 3：AI 助手生成感谢文案 & 周报（Qwen / Qwen-Agent 发力点）**

1.  创作者点击「生成本周感谢文案」按钮。
    
2.  后端调用 **Qwen-Agent**：
    
    -   工具 1：从数据库/链上读取最近打赏数据；
        
    -   工具 2：根据金额 & 留言，为主要打赏人生成个性化感谢语；
        
    -   工具 3（可选）：生成一段「本周支持报告」摘要。
        
3.  前端展示 AI 草稿，创作者一键复制或直接分享到 X / GitHub / Discord。
    

> **Qwen / Qwen-Agent 发力点**：
> 
> -   不仅能「写文案」，还能基于工具调用链上数据，做真正的运营助手：分析谁是核心粉丝、他们在哪条链、最近打赏趋势如何。
>     
