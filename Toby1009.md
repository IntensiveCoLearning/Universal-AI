...
關於題目，有跟朋友組了個隊討論討論，心中還是沒個底。

這次有兩個賽道，defi和ai，我覺得defi比較好加入，所以會優先選defi

defi我目前的想法就跟上次一樣，做個通用隱私Railgun，或者玩個生活的小工具

Railgun上次介紹過了，這次介紹了個生活的。

都是建立在同個思維底下：「支付上的鏈無感知」

啥意思呢？就是發送與接收端都不需要思考對方在什麼鏈上，我在什麼鏈上。

從這個思維下我想出了幾種題目：

-   通用stream.gift：參考[https://sonic.stream.gift/](https://sonic.stream.gift/)
    
-   通用zkSend：參考[https://sdk.mystenlabs.com/zksend](https://sdk.mystenlabs.com/zksend)
    
-   商家掃碼：昨天講的
    

stream.gift的通用版的話，就是可以讓收款方選擇收款地址和代幣就好，然後發送方不需要改變，想送啥就送啥，然後透過Zetachain上面轉換過去，對方收到後在直播畫面顯示出來。

zkSend也是這個套路，zkSend是可以傳連結給對方，對方收到連結就可以收款，那咱Zetachain就可以讓這個流程變得有趣，可以變成送款方選啥鏈選啥資產都沒差，收款者也是可以選啥鏈都行，這代表啥，我送禮，我可以送成很多成很多種資產，給很多條鏈
...
## 題目想法 - RailGun X Zetachain 通用隱私

痛點：目前在跨鏈上，很輕鬆就能知道對方要轉去目標鏈的地址，很沒隱私性

像是下面，以Circle把USDC從Sui跨鏈到Ethereum為例，可以很清楚的看到目標地址

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/Toby1009/images/2025-12-05-1764939488993-image.png)

接著一追蹤就能追到流向，沒啥隱私

![Screenshot 2025-12-05 at 8.59.55 PM.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/Toby1009/images/2025-12-05-1764939622266-Screenshot_2025-12-05_at_8.59.55_PM.png)

尤其是最近V神老是說隱私，而且也提到Railgun，所以想在上面試試看，Railgun也開源，但也不知道能不能在兩週部署與實現出來  
Railgun網站：

-   官網：[https://railgun.org/](https://railgun.org/)
    
-   Wiki：[https://docs.railgun.org/wiki](https://docs.railgun.org/wiki)
    
-   Developer Guide：[https://docs.railgun.org/developer-guide](https://docs.railgun.org/developer-guide)
    
-   Typscript啥的：[https://github.com/Railgun-Community](https://github.com/Railgun-Community)
    
-   合約、ZK電路啥的：[https://github.com/Railgun-Privacy](https://github.com/Railgun-Privacy)
    

可能有人會問：這跟tornado cash差異？

這個跟tornado cash差在沒有混亂用戶資金，並且有一套**Private Proofs of Innocence** ，防止惡意資金入場，有惡意資金入場後就會生成ZKP，以供後續調查

Railgun的好處就是，能夠隱藏使用者交易、以及交易金額和訊息。

他咋做的呢？

-   池子，UTXO，以外部來看，就池子的代幣轉去外面交易而已，不知道是池子裡的誰
    
-   Waku + Broadcast，隱藏使用者交易簽名IP以及支付gas而暴露public wallet
...
![Screenshot 2025-12-05 at 9.17.14 PM.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/Toby1009/images/2025-12-05-1764940642242-Screenshot_2025-12-05_at_9.17.14_PM.png)

後續主要要做的事確認流程，Zetachain通用流程，咋串Universal，咋串多鏈，這都是問題，以及部署是否有問題，還有Typescript sdk等等

其實還有想過像是多鏈商家QRcode支付，
...
用戶在用戶層輸入文字說想幹麻，接著在llm.ts解析意圖（解析意圖層），然後按下確認意圖後會到blockchain.ts的executeIntent，這邊會進行功能分發，確認跨鏈與否，有不同任務。

![Screenshot 2025-12-04 at 8.23.34 PM.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/Toby1009/images/2025-12-04-1764860664668-Screenshot_2025-12-04_at_8.23.34_PM.png)

下面簡單解釋了流程，以及技術Stack

![Screenshot 2025-12-04 at 8.53.05 PM.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/Toby1009/images/2025-12-04-1764860682581-Screenshot_2025-12-04_at_8.53.05_PM.png)

在llm.ts中有個細節是使用Next.js API路由調用LLM，有幾個好處：

-   安全性，保護API KEY `const apiKey = process.env.QWEN_API_KEY//在blockchain.ts` ，如果直接在LLM掉用API要在client端包含API KEY，使用路由調用就能隱藏
    
-   避免CORS跨域問題，瀏覽器的同源策略會阻止跨域請求，就我在localhost，調用別的地方的API就會這樣，而如果用Next.js API路由就能自由调用任何外部 API，沒有 CORS 限制
