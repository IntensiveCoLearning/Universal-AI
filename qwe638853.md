...
明天開始黑客松，今天繼續研究Zk、Railgun protocol! 簡單打個卡
<!-- DAILY_CHECKIN_2025-12-07_END -->

# 2025-12-06
<!-- DAILY_CHECKIN_2025-12-06_START -->

今天同樣繼續研究Railgun，看看是否能將他移植出一個在Zetachain上的版本，所以今天簡單打個卡不特別寫東西
...
研究了一下市面上的隱私協議，目前主流的隱私方案大多都**只能在單鏈上運作**，或者乾脆**自己建一條新的鏈**，用底層共識來處理隱私。所以要是如果能將隱私轉帳結合到Zetachain上，那不就能大大的增強隱私轉帳這個領域的使用者體驗了嗎  
  
因為 ZetaChain 本身就有跨鏈訊息與跨鏈資產的能力，理論上只要把隱私池、UTXO note、stealth address 這些機制整合上去，就有可能打造出一個**一次部署、多鏈共享的隱私池**

當然，目前這些想法都還在比較早期的構想階段，很多細節還沒完全想清楚，也還需要先跟隊友討論一下可行性、參賽方向、技術模型的複雜度、以及是否適合我們的時間與資源。
...
那今天就將昨天實作簡單轉帳Agent與Zetachain的Messaging串接

### Zetachain messaging Agent

那當我們跟著[Messaging範例](https://www.zetachain.com/docs/developers/tutorials/messaging)部屬完Messaging合約，並且將兩條鏈互相做綁定後，就可以開始建構我們的tool了，那我的基本定義如下面的code，Agent只需判斷出訊息(`message`)、要寄送的Token(`targetToken`)、要寄送訊息的目標鏈(`targetChain`)、數量(`amount`)以及發送訊息的鏈`sourceChain`就可以

```
@register_tool('simple_call_message')
class SimpleTokenDeposit(BaseTool):
    description = 'Send native token and call sendMessage() on messaging contract'
    parameters = [
        {
            'name': 'message',
            'type': 'string',
        ...
        },
        {
            'name': 'targetToken',
            'type': 'string',
            'description': 'The target token symbol to send (e.g., ETH).',
            'required': True
        },
        {
            'name': 'targetChain',
            'type': 'string',
            'description': 'The target chain name (e.g., basesepolia).',
            'required': True
        },
        {
            'name': 'amount',
            'type': 'string',
            'description': 'The amount of the native token to send (in ETH-like units).',
            'required': True
        },
        {
            'name': 'sourceChain',
            'type': 'string',
            'description': 'The source chain name (e.g., sepolia).',
            'required': True
        }
    ]
```

那另外我們還需準備合約的ABI，因為我們只需要調用Messaging合約裡面的`sendMessage()`，所以我們只需要準備`sendMessage()`的ABI即可
...
那準備好ABI，就可以用Web3套件去建立合約實例，並調用合約內的方法，但要注意這邊由於我們只有準備`sendMessage()`的ABI，所以目前只能調用這個方法，那簡略的步驟如下，處理裡面參數就沒特別放了
...
# 建立Messaging合約實例
messaging = w3.eth.contract(
    address=w3.to_checksum_address(src_messaging_address),
    abi=SEND_MESSAGE_ABI,
)

# 建構交易
tx = messaging.functions.sendMessage(
	receiver_bytes,
	target_token_addr,
	data_bytes,
	gas_limit_default,
...
那我們直接看看效果，下面提問 "Sepolia送"Hello"到Base sepolia，並夾帶0.0001ETH"  

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/qwe638853/images/2025-12-04-1764839346240-image.png)

  
Agent顯示交易成功  

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/qwe638853/images/2025-12-04-1764839364039-image.png)

  
那也可以用下面指令去查詢是否跨鏈成功

```
npx zetachain query cctx --hash b97c2c1519ac2ad886dc25f4782e2b26534a2805b993ad70dd620a8d733eb35
```

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/qwe638853/images/2025-12-04-1764839393521-image.png)

那拿到CCTX後到Zetachain的區塊鏈瀏覽器查看詳情，也是成功了

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/qwe638853/images/2025-12-04-1764839418444-image.png)

### 小結

嘗試將Zetcahin與Agent成功結合了，雖然只是簡單的串接範例合約而已，但後面操作更複雜的操作時也有個基底的code可以去參考修改了
...
今天做個簡單的跨鏈轉帳Agent，這幾天有點忙就不做前端了哈哈

### 轉帳Agent

那我這個轉帳Agent，做得稍微比較偷懶一點，私鑰直接放在env去送交易，但更好的做法是做個前端->連接錢包->建構交易數據->用戶送出交易->完成轉帳

那接下來寫一個tool讓agent可以建構交易並送出，那這裡我用了Python與鏈上互動常用的一個套件叫Web3，那基本的定義如下，Agent只需判斷出數量跟接收地址即可，但如果要做個更彈性的話，可以讓Agent決定用戶想要在什麼鏈或是什麼Token，那這裡我就只做簡單的在Sepolia上做Native Token的轉帳

```
@register_tool('simple_token_deposit')
class SimpleTokenDeposit(BaseTool):
    description = 'Send a simple native token deposit using web3.py to the ZetaChain Connector contract on the source chain.'
    parameters = [
        {
            'name': 'amount',
            'type': 'string',
            'description': 'The amount of the native token to send.',
            'required': True
        },
        {
            'name': 'recipient',
            'type': 'string',
            'description': 'The recipient address on the source chain.',
            'required': True
        }
    ]
```

那實際的Call邏輯如下，其實Web3跟Ether.js用法蠻相似，都是先建立個Provider，建構交易，簽署交易，流程與概念都是互通的
...
def call(self, params: str, **kwargs) -> str:
        try:
            ...
            source_rpc_url = os.getenv("RPC_URL")
            private_key = os.getenv("PRIVATE_KEY")

            if not (private_key and recipient):
                return json.dumps({"error": "Missing env vars: PRIVATE_KEY or recipient"}, ensure_ascii=False)

            w3 = Web3(Web3.HTTPProvider(source_rpc_url))
            if not w3.is_connected():
                return json.dumps({"error": f"Failed to connect to RPC: {source_rpc_url}"}, ensure_ascii=False)

            sender_address = w3.eth.account.from_key(private_key).address
            amount_wei = w3.to_wei(amount_str, 'ether') # 假設是 ETH 類型的原生代幣
            ...
            return json.dumps({
                ...
                "note": f"Sent {amount_str} native token from {sender_address} to ZetaChain Connector on source chain."
            }, ensure_ascii=False)

        except Exception as e:
            return json.dumps({"error": f"Web3 transaction failed: {str(e)}"}, ensure_ascii=False)
```

最後是實際測試的樣子

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/qwe638853/images/2025-12-03-1764742729189-image.png)

那可以到Etherscan上去查詢結果，可以看到真的成功了

![image.png](https://raw.githubusercontent.com/IntensiveCoLearning/Universal-AI/main/assets/qwe638853/images/2025-12-03-1764742756333-image.png)

## 小結

今天算是熟悉了下Python串接區塊鏈的套件及工具，畢竟平常串區塊鏈還是用Ether.js居多，比較少用Python去串
