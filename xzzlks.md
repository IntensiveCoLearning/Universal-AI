...
| 日期 | 里程碑（阶段目标） | 各角色当日关键产出 |
| 第1天 | 需求对齐（准备阶段） | 1. 全团队：召开需求评审会，确认“捐赠-追溯-NFT”全流程细节；2. 技术负责人：输出技术架构图；3. 前端：完成3个核心页面线框图 |
| 第2天 | 环境与资源准备（准备阶段） | 1. 合约：搭建ZetaEVM开发环境，下载ERC721模板；2. 后端：搭建开发框架，配置数据库；3. AI：获取Qwen-Agent调用权限，准备测试数据集；4. 前端：搭建Vue/React项目框架 |
| 第3天 | 基础模块开发启动（开发阶段） | 1. 合约：完成捐赠合约核心函数（接收捐款、记录明细）；2. 后端：开发“项目列表查询”接口；3. 前端：完成项目列表页开发；4. AI：完成意图解析工具初版开发 |
| 第4天 | 核心功能开发1（开发阶段） | 1. 合约：完成NFT合约（铸造、元数据查询）；2. 后端：开发“捐赠请求处理”接口；3. 前端：完成捐赠输入页开发；4. AI：优化意图解析，支持3种核心句式 |
| 第5天 | 核心功能开发2（开发阶段） | 1. 合约：完成两合约关联逻辑（捐赠触发NFT铸造）；2. 后端：开发“NFT发放查询”接口；3. 前端：完成NFT展示页开发；4. AI：开发追溯报告初版逻辑 |
| 第6天 | 链上部署与接口对接（开发阶段） | 1. 合约：将两合约部署至ZetaEVM测试网，输出ABI；2. 后端：对接合约与Qwen-Agent，完成接口联调；3. 前端：完成页面跳转逻辑开发；4. AI：完成追溯报告与链上数据对接 |
| 第7天 | 模块单元测试与修复（开发阶段） | 1. 合约：编写合约单元测试，修复逻辑漏洞；2. 后端：进行接口压力测试，优化响应速度；3. 前端：修复页面样式与交互Bug；4. AI：测试意图解析准确率，达标＞90% |
| 第8天 | 全流程联调1（基础场景）（联调阶段） | 1. 全团队：测试“Ethereum链USDC捐赠”完整流程；2. 后端+合约：排查跨链交易延迟问题；3. 前端+AI：确认解析-报告全链路流畅；4. 测试：记录首次联调Bug清单 |
| 第9天 | 全流程联调2（多链场景）（联调阶段） | 1. 全团队：测试“Polygon/Base链”捐赠场景；2. 开发：按优先级修复Bug（如NFT铸造不同步）；3. 前端：优化多链捐赠后的页面反馈；4. AI：适配多链场景的追溯报告生成 |
| 第10天 | Bug修复与回归测试（联调阶段） | 1. 开发：修复剩余Bug，进行回归测试；2. 测试：验证所有Bug修复效果；3. 全团队：模拟极端场景（如网络波动）测试稳定性；4. 后端：配置接口熔断机制 |
| 第11天 | 体验优化与用户试用（优化阶段） | 1. 前端：优化页面加载动画、按钮反馈；2. 后端：优化链上数据查询速度；3. 全团队：邀请用户试用，收集反馈；4. AI：根据用户输入优化解析模型 |
| 第12天 | 迭代优化与功能完善（优化阶段） | 1. 开发：根据用户反馈优化核心功能（如简化NFT查看路径）；2. 前端：适配不同浏览器；3. 后端：补充接口日志，便于问题排查；4. 合约：整理合约注释，提升可读性 |
| 第13天 | 演示环境部署与彩排（最终准备） | 1. 后端+前端：部署演示环境，配置备用RPC节点；2. Pitch负责人：完善PPT，熟悉Demo脚本；3. 全团队：进行首次演示彩排，记录问题；4. 技术：准备应急方案（如演示失败切换录屏） |
| 第14天 | 最终彩排与提交准备（最终准备） | 1. 全团队：进行2次以上最终彩排，模拟评委提问；2. Pitch负责人：优化宣讲语气与节奏；3. 开发：检查演示环境，备份项目代码；4. 全团队：整理提交材料（代码、PPT、说明文档） |

### 1-3分钟Demo脚本草稿（用户视角+技术亮点标注）

| 演示环节 | 操作与讲解内容技术亮点标注 |   |
| 开场：项目价值说明 | （屏幕展示项目首页）大家好，今天带来ZetaGiving——AI全链慈善捐赠平台。它能让加密用户一键用跨链资产做公益，资金流向全透明，还能获得专属NFT纪念，彻底解决加密公益“操作难、不信任”的痛点。 | 无技术标注，聚焦社会价值，快速引发共鸣 |
| 步骤1：自然语言发起捐赠 | （操作：点击“我要捐赠”，在输入框输入“用Polygon链50 USDC捐给土耳其地震救助”，点击确认）大家看，无需手动选链、填合约地址，AI会自动解析我的需求。（屏幕显示解析结果：资产链Polygon、金额50USDC、项目匹配成功） | 这里是Qwen-Agent发力点：通过改造的ParseDonationIntent工具，精准提取“链-金额-项目”核心参数，新手也能轻松操作。 |
| 步骤2：一键完成跨链捐赠 | （操作：点击“确认捐赠”，弹出钱包授权窗口，完成签名）授权后无需其他操作，平台会自动处理跨链。（屏幕显示“捐赠成功”，并展示交易哈希）现在50USDC已从我的Polygon地址，跨链转到项目在Ethereum的专属合约里。 | 这里是ZetaChain发力点：利用Omnichain特性，实现原生资产跨链转移，无需用户手动封装ZRC-20，同时捐赠合约确保资金仅用于该项目。 |
| 步骤3：获取NFT与追溯资金 | （操作：点击“我的NFT”，屏幕展示“地震救助爱心NFT”）捐赠完成瞬间，专属NFT已自动到账，元数据里有我的捐赠记录。（点击NFT上的“查看流向”，展示自然语言报告）报告清晰显示：我的资金→项目合约→已用于采购20顶帐篷，每一步都可查链上哈希。 | 双技术发力：1. Qwen-Agent将链上哈希转化为自然语言报告，降低查询门槛；2. ZetaChain合约记录全流程数据，配合NFT实现“捐赠-纪念-追溯”闭环。 |
| 结尾：项目价值总结 | ZetaGiving用AI降低门槛，用ZetaChain保障透明，让加密资产真正赋能慈善。谢谢大家！ | 总结技术与社会价值，简洁有力 |
...
以“**自然语言交互+全链透明+NFT激励**”为核心，让加密资产真正成为慈善事业的助力，同时体现“AI+区块链”的技术价值。

### 目标用户 / 场景：

**捐赠者**：持有ETH/USDC等加密资产、有公益意愿、关注捐款透明度、希望为加密行业和慈善行业做贡献；痛点：跨链捐赠操作复杂、担心项目诈捐、捐款后无直观反馈；

**被捐赠者**：个人：因自然灾害、重大疾病、战争、意外陷入困境的群体；组织：正规备案的公益基金会、灾害救助NGO；痛点：捐赠资产来源分散、资金使用需公开证明、跨链收款不便；

**核心场景**：场景1：捐赠者说“用Polygon链50 USDC捐给土耳其地震救助”，一键完成跨链捐赠并收到“爱心救助者”NFT；场景2：大病患者发起求助，平台生成专属合约，每笔捐款实时上链，提现时需标注“手术费”用途；场景3：捐赠者通过NFT详情页，查看“自己钱包→救助合约→医院账户”的完整链上记录。
...
-   **功能3：定制化捐赠NFT自动发放**：捐赠完成后，基于“金额+项目类型”自动铸造专属NFT（如100USDC对应“白银爱心”，500USDC对应“黄金爱心”）；NFT元数据包含捐赠时间、项目名称、追溯链接，兼具纪念意义与查询功能，支持在Zeta生态钱包展示。
    

### 技术路线（ZetaChain + Qwen 协同）
...
Day10：ParseSwapIntent工具（改造为捐赠意图解析工具）；Day11：ZetaSwapInterface中间层（保留参数校验，修改为公益合约路由）；Day12：ZetaTransactionExecutor（适配ZetaChain跨链交易与合约调用）；ZetaChain官方Omnichain合约模板；OpenZeppelin ERC721合约模板；开源公益平台UI组件； DeFi项目NFT看板模板

注：目前仅为初步想法后续可能稍作调整，且本人能力有限时间不充裕，该项目可扩展性强，复用性强，整体实现可能过于宏大，初步仅可实现部分功能。
